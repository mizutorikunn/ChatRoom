...
// type
// *********************
declare type userInfo = { id: string; nickName: string };
...
declare type msg = {
  ...
  nickName?: string;
  color?: string | null;
  privateChat?: { id: string; nickName: string };
  privateTarget?: userInfo | null;
};

@Component({})
export default class CheatRoom extends Vue {
  // *********************
  // Properties
  // *********************

  private nickName: string = localStorage.nickName;
  //socket.io的客户端
  ...
  //计时器ID，计时结束时istyping会改为false
  private timeout: number | null = null;
  //聊天模式，默认是全局聊天，可切换为私密聊天(privateMyself)
  private mode: string = "global";
  //私密聊天时的对象的socket id
  private privateTarget: userInfo | null = null;
   ...
   * 切换到公共聊天室
   */
  private switchToGobalChatRoom(): void {
    this.mode = "global";
  }

  /**
   *  切换到私密聊天模式
   * @param userData 私密对象的id及名称
   */
  private swithToPrivateChatRoom(userData: userInfo): void {
  ...
  private mounted(): void {
    ...
    this.socket.on("typing", (nickName: string) => {
      //记录正在输入的用户
    ...
    //监听私密聊天事件
    this.socket.on("privateChat", (msg: msg) => {
      this.messageList.push({
        type: "privateChat",
        content: msg.content,
        nickName: msg.nickName,
